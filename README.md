...
### PigGo 儿童动画增强识别
  ...
  > V2 当前开发版本：`0.4.2`（适配 MoviePilot V2.15.6，待真实宿主验收）

  从一个或多个 PigGo RSS 或用户粘贴的磁力/下载链接建立候选，经用户确认后调用 MoviePilot 下载器，并按 hash 跟踪下载、扫描真实内容包中的 NFO、图片和文件名。V2.15.6 起提供联邦配置页、详情页和侧栏全页工作台，可在下载前修正候选身份，并在冲突证据上批准或忽略任务；批准后仍需再次确认才执行整理。V3 另外注册稳定的 `piggokids` 本地媒体来源。
