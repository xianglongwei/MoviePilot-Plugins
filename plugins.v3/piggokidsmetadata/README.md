...
当前版本 `0.3.1` 在 RSS/粘贴链接到下载后识别闭环上增加只读 TMDb 严格匹配和人工冲突审核 API，并修复并发提交、逐文件整理结算和 RSS 网络边界。它不会修改 TMDb，也不会直接移动文件；整理通过 MoviePilot 的整理链完成，且自动整理默认关闭。

已实现：

- 配置一个或多个 HTTP(S) RSS/Atom 地址，定时或手工刷新并稳定去重。
- 通过鉴权 API 查看不含查询密钥的 RSS 抓取状态，并修正、忽略或恢复尚未进入任务流的候选。
