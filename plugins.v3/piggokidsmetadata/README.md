...
当前版本 `0.3.3` 在 RSS/粘贴链接到下载后识别闭环上增加只读 TMDb 严格匹配和人工冲突审核 API，并修复并发提交、逐文件整理结算、RSS 网络边界、完成任务从下载器活动列表消失后的历史恢复以及双进度尺度兼容。它不会修改 TMDb，也不会直接移动文件；整理通过 MoviePilot 的整理链完成，且自动整理默认关闭。
